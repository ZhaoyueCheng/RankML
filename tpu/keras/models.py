...
def DLRM(config):
    ...
    sparse_inputs = [keras.Input(shape=(), dtype='int32', name=f'sparse_feature_{i}') for i in range(num_sparse_features)]
    ...
    for i in range(num_sparse_features):
        embedding_layer = keras.layers.Embedding(input_dim=vocab_sizes[i], output_dim=embedding_dim, name=f'embedding_{i}')
        sparse_embedding = embedding_layer(sparse_inputs[i])  # Shape: (batch_size, embedding_dim)
        sparse_embeddings.append(sparse_embedding)

    # Interactions between dense and sparse embeddings
    all_embeddings = [dense_embedding] + sparse_embeddings  # List of tensors with shape (batch_size, embedding_dim)
    # Concatenate interactions
    x = keras.layers.Concatenate()(all_embeddings)  # Shape: (batch_size, num_interactions)
