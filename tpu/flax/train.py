...
def train_step(state, batch):
    def loss_fn(params):
        logits = state.apply_fn({'params': params}, batch['dense_features'], batch['sparse_features'])
...
def eval_step(state, batch):
    logits = state.apply_fn({'params': state.params}, batch['dense_features'], batch['sparse_features'])
    loss = bce_with_logits_loss(logits, batch['labels'])
    metrics = compute_metrics(logits, batch['labels'])
    metrics['loss'] = loss
    return metrics
